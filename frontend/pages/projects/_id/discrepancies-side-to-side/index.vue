<template>
  <div>
    <v-card>
      ...
      <v-card-title>
        Comparação de Anotações Lado a Lado
      </v-card-title>
      <v-card-text>
        <v-row>
          <v-col cols="12" md="6">
    ...
    <v-card v-if="showComparison" class="mt-4">
      <v-card-title>
        Comparação de Anotação: {{ selectedExample ? exampleNameMap[selectedExample] || selectedExample : '' }}
        <v-spacer></v-spacer>
      ...
      <v-card-subtitle v-if="exampleText" class="pb-0">
        Texto Original
      </v-card-subtitle>
  ...
  methods: {
    ...
    handleError(error: any) {
      if (error.response && error.response.status === 400) {
        ...
        this.errorMessage = 'O banco de dados está lento ou indisponível. Por favor, tente novamente mais tarde.'
      }
    }
  }
})

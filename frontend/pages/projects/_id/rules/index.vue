  ...
  <v-card>
    ...
    <v-card-text>
      ...
      <v-alert v-if="hasActiveVoting && isAdmin" type="info" class="mb-4">
        Existe uma votação ativa (Versão {{ activeVotingConfig?.version }}). Não é possível criar uma nova votação até que todas as regras da votação atual estejam finalizadas.
      </v-alert>
            ...
            <v-row class="mb-6">
              <v-col v-for="rule in pendingRules" :key="rule.id" cols="12" sm="6" md="4">
                <v-card outlined class="mb-4">
                  ...
                  <v-card-actions>
                    <v-btn small color="success" :disabled="rule.id in answeredRules"
                      @click="vote(rule.id, true)">Sim</v-btn>
                    <v-btn small color="error" :disabled="rule.id in answeredRules"
...
export default Vue.extend({
  ...
  async fetch() {
    ...
    try {
      ...
      this.pendingRules = this.rules.filter(rule => !this.answeredRules[rule.id]) as AnnotationRuleItem[];
    } catch (error) {
    ...
    async checkAndCloseCompletedVotings() {
      try {
        ...
        for (const config of activeConfigs) {
          ...
          const allFinalized = configRules.length > 0 && 
            configRules.every(rule => rule.is_finalized);

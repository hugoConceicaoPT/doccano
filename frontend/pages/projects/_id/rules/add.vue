  ...
  methods: {
    async checkActiveVotingConfiguration() {
      try {
        ...
        const activeConfigs = existingConfigs.filter((config: { is_closed: boolean }) => !config.is_closed);
        
        if (activeConfigs.length > 0) {
          for (const activeConfig of activeConfigs) {
            const rules = await this.annotationRuleService.list(this.projectId);
            const configRules = rules.filter((rule: { voting_configuration: number }) => 
              rule.voting_configuration === activeConfig.id);
            
            const allRulesFinalized = configRules.length === 0 || 
            ...
            if (allRulesFinalized) {
              ...
              this.errorMessage = 'Não é possível configurar uma nova votação pois existe uma votação ativa com regras não finalizadas.';
              setTimeout(() => {
    ...
    async determineNextVersion() {
      try {
        ...
        if (existingConfigs && existingConfigs.length > 0) {
          const existingVersions = existingConfigs.map((config: { version: number }) => config.version)
            .sort((a: number, b: number) => a - b);
          
          const uniqueVersions = [...new Set(existingVersions)];
          if (uniqueVersions.length !== existingVersions.length) {
            console.warn('Atenção: Foram detectadas versões duplicadas nas configurações de votação.');
          }
          
          const maxVersion = Math.max(...uniqueVersions as number[]);
          nextVersion = maxVersion + 1;
          
          console.log(`Versões existentes: ${uniqueVersions.join(', ')}. Próxima versão: ${nextVersion}`);
        }

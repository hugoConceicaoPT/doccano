      ...
      <v-card class="pa-4 mb-4 elevation-1">
        <v-form>
          <v-row dense>
            ...
            <!-- Anotação -->
            <v-col cols="12" md="4">
              <v-select v-model="selectedAnnotations" :items="formattedAnnotations" item-text="text" item-value="value"
                label="Annotation" dense outlined multiple hide-details placeholder="Select" />
            </v-col>

            <!-- Anotador -->
            <v-col cols="12" md="4">
              <v-select v-model="selectedAnnotators" :items="annotators" label="Annotator" dense outlined hide-details
...
export default Vue.extend({
  ...
  computed: {
          ...
          if (matchingAnswers.length > 0) {
            dist[key] = {
              ...
              answers: Object.fromEntries(
                matchingAnswers.map(a => [a, entry.answers[a]])
              ),
    ...
    filteredCategoryPercentage() {
      ...
      for (const [key, value] of Object.entries(this.categoryPercentage)) {
        const matchesAnnotation = this.selectedAnnotations.length === 0 || this.selectedAnnotations.includes(Number(key));
  ...
  async created() {
    try {
      ...
      this.annotators = this.members.filter((m: MemberItem) => m.isAnnotator).map(item => item.name)
